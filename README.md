...
In the project where you use your agent, install the skill with the
[`skills` CLI](https://github.com/vercel-labs/skills):

```sh
npx skills add d-date/swift-jev --skill jev -a codex -a claude-code -a cursor -a antigravity-cli -a antigravity -a github-copilot
```

This installs into the current project. Use `-g` for a user-wide installation,
or keep only the `-a` options for the agents you use. For example, to install
for Codex and Claude Code across your projects:

```sh
npx skills add d-date/swift-jev --skill jev -g -a codex -a claude-code
```

This checkout already links the skill at `.agents/skills/jev` and
`.claude/skills/jev`, so no installation is needed here. Ask your agent to use
the `jev` skill and describe the state and questions to evaluate. Give the agent
process access to `TYPESAFE_API_KEY` or an API key file; the skill does not store
the key.
